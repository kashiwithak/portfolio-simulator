...
if df_preview is not None:
    ...
    progress_df = pd.DataFrame({
        "Label": ["Achieved", "Remaining"],
        "Value": [min(total_profit, profit_goal), max(0, profit_goal - total_profit)]
    })
    chart = alt.Chart(progress_df).mark_bar().encode(
        x=alt.X("sum(Value):Q", title="Profit"),
        y=alt.Y("Label:N", sort=["Remaining", "Achieved"]),
